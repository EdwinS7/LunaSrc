...
//What the fuck is this pasted shit lmfao. (whatever)
void lagcompensation::FixLandingAnimations(player_t* m_pPlayer, adjust_data* LagRecord, adjust_data* PreviousLagRecord)
{
	float LandTime = 0.f;

	if (LagRecord->layers[4].m_flWeight > 0.f && LagRecord->layers[4].m_flWeight < 1.f) {
		auto SequenceAcitivy = m_pPlayer->sequence_activity(LagRecord->layers[4].m_nSequence);

		if (SequenceAcitivy == 988 || SequenceAcitivy == 989) {
			auto LandingTime = LagRecord->layers[4].m_flCycle / LagRecord->layers[4].m_flPlaybackRate;

			if (LandingTime > 0.f)
				LandTime = LagRecord->simulation_time + LandingTime;
		}

		m_pPlayer->m_vecVelocity().z = 0.f;
	}
}
...
void lagcompensation::update_player_animations(player_t* e)
{
	...
	if (cfg::g_cfg.ragebot.resolver && !player_info.fakeplayer && g_ctx.local()->is_alive() && !cfg::g_cfg.legitbot.enabled)
	{
		if (e->m_iTeamNum() == g_ctx.local()->m_iTeamNum() && m_cvar()->FindVar("mp_teammates_are_enemies")->GetInt() == 0) {
			g_ctx.globals.side = 0;
			g_ctx.globals.desync = 0.0f;
			goto skip_resolve;
		}

		//For setting the goal feet yaw below, be careful with the amounts. it can break safepoints!
		//Setup our shit and get ready.
		CPlayerResolver[idx].Init(e, record, previous_goal_feet_yaw[idx], e->m_angEyeAngles().x);

		animstate->m_flGoalFeetYaw = previous_goal_feet_yaw[idx];


		g_ctx.globals.updating_animation = true; e->update_clientside_animation(); g_ctx.globals.updating_animation = false;

		previous_goal_feet_yaw[idx] = animstate->m_flGoalFeetYaw;
		memcpy(animstate, &state, sizeof(c_baseplayeranimationstate));
		CPlayerResolver[idx].gfy_default = animstate->m_flGoalFeetYaw; //oop
		animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y);
		...
		//Run our actual resolver.
		CPlayerResolver[idx].RunResolve(e);
	}
	else { //Fix these for logs
		g_ctx.globals.side = 0;
		g_ctx.globals.desync = 0.0f;
	}
...
skip_resolve:
	...
	memcpy(CPlayerResolver[idx].previous_layers, animlayers, e->animlayer_count() * sizeof(AnimationLayer));
...
void lagcompensation::update_sequence() {
	...
	if (net_channel) {
		if (net_channel->m_nInSequenceNr > m_last_incoming_sequence_number) {
			...
			sequences.push_front(incoming_sequence_t(net_channel->m_nInReliableState, net_channel->m_nOutReliableState, net_channel->m_nInSequenceNr, m_globals()->m_realtime));
		}
...
//Extended backtrack stuff.
void lagcompensation::add_latency(INetChannel* net_channel) {
	for (auto& seq : sequences) {
		if (m_globals()->m_realtime - seq.m_current_time >= cfg::g_cfg.misc.extended_backtracking_value / 1000.f) {
