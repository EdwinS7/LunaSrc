...
void legit_bot::do_smooth(const Vector& currentAngle, const Vector& aimAngle, Vector& angle)
{
	...
	aim_punch *= (cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].rcs / 100.f);

	Vector delta = aimAngle - (currentAngle + aim_punch);
	math::normalize_angles(delta);

	if (cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].smooth_type == 1) {
...
{
	static auto LineGoesThroughSmokeFn = (bool(*)(Vector vStartPos, Vector vEndPos))util::FindSignature(CLIENT_DLL, crypt_str("55 8B EC 83 EC 08 8B 15 ? ? ? ? 0F"));
...
void legit_bot::createmove(CUserCmd* cmd)
{
	...
	if (cfg::g_cfg.legitbot.autopistol)
		auto_pistol(cmd);

	if (cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].target_switch_delay > m_globals()->m_realtime)
	{
		point = Vector(0, 0, 0);
		target = nullptr;
		autofire_delay = 0;

		if ((key_binds::get().get_key_bind_state(1) || key_binds::get().get_key_bind_state(0)) && !cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].rcs_type)
			do_rcs(cmd);
	...
	//flash check 
	if (cfg::g_cfg.legitbot.disasblers[0] && g_ctx.local()->m_flFlashDuration() >= 0.1f)
	{
		autofire_delay = 0;
	...
	//air check 
	if (cfg::g_cfg.legitbot.disasblers[1] && !(g_ctx.local()->m_fFlags() & FL_ONGROUND))
	{
		autofire_delay = 0;
	...
	//smoke check 
	if (cfg::g_cfg.legitbot.disasblers[2] && in_smoke(g_ctx.globals.eye_pos, target->hitbox_position(0)))
	{
		autofire_delay = 0;
	...
	//scope check
	if (weapon->is_sniper() && cfg::g_cfg.legitbot.disasblers[3] && !g_ctx.local()->m_bIsScoped())
	{
		if (should_do_rcs)
			do_rcs(cmd);
	...
	if (!g_ctx.local()->m_iShotsFired() && key_binds::get().get_key_bind_state(0) && autofire_delay < cfg::g_cfg.legitbot.autofire_delay)
	{
		autofire_delay += 1;
		return;
	}
	...
	if (cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].rcs_type || should_do_rcs)
		do_rcs(cmd);

	auto hitchanced = hitchance(target, aim_angle, point, targets[target->EntIndex()].hitbox) >= cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].autofire_hitchance;
...
{
	if (g_ctx.local()->m_iShotsFired() < 2 && cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].fov > 0)
		fov_t = cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].fov;
...
void legit_bot::calc_smooth()
{
	if (g_ctx.local()->m_iShotsFired() < 2)
		smooth_t = cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].smooth;

	else if (g_ctx.local()->m_iShotsFired() >= 2 && cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].custom_rcs_smooth >= 1.f)
		smooth_t = cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].custom_rcs_smooth;
...
void legit_bot::do_rcs(CUserCmd* cmd)
{
	...
	if (weapon->m_iItemDefinitionIndex() == WEAPON_REVOLVER
		|| cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].rcs <= 0
		|| weapon->is_non_aim()
...
void legit_bot::auto_pistol(CUserCmd* cmd)
{
	...
	if (!weapon->is_pistol() || weapon->m_iItemDefinitionIndex() == WEAPON_REVOLVER || weapon->m_iClip1() <= 0)
		return;
	...
	*/
	...
	const auto activeWeapon = g_ctx.globals.weapon;
	static bool is_firing = false;
	static DWORD delay = GetTickCount();

	//cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].target_switch_delay > m_globals()->m_realtime
	if (activeWeapon && activeWeapon->is_pistol() && delay > cfg::g_cfg.legitbot.auto_pistol_delay)
	{
		if (activeWeapon->m_iItemDefinitionIndex() == WEAPON_REVOLVER && is_firing)
			cmd->m_buttons &= ~IN_ATTACK2;
		else if (is_firing)
			cmd->m_buttons &= ~IN_ATTACK;
	}
...
void legit_bot::find_best_point(CUserCmd* cmd, float fov_v)
{
	...
	if (cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].priority == 0)
	{
		for (auto bone : { CSGOHitboxID::Head, CSGOHitboxID::Pelvis, CSGOHitboxID::Chest, CSGOHitboxID::UpperChest, CSGOHitboxID::Stomach })
		{
			float distance = 0.f;
			float fov = FLT_MAX;
			...
			if (cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].fov_type == 1)
			...
			if (!fire_data.visible && cfg::g_cfg.legitbot.disasblers[4])
				continue;

			if (!fire_data.visible && 1 > 0 && fire_data.damage < 1)
				continue;
	...
	else if (cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].priority == 1) // head only
	{
		float distance = 0.f;
		float fov = FLT_MAX;
		Vector ang = Vector(0, 0, 0);
		...
		if (!fire_data.visible && !cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].awall_dmg)
			return;

		if (!fire_data.visible && cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].awall_dmg > 0 && fire_data.damage < cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].awall_dmg)
			return;
	...
	else if (cfg::g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].priority == 2) // nearest body
	{
		for (auto bone : { CSGOHitboxID::Pelvis, CSGOHitboxID::Chest, CSGOHitboxID::UpperChest, CSGOHitboxID::Stomach }) {
			float distance = 0.f;
			float fov = FLT_MAX;
			Vector ang = Vector(0, 0, 0);
...
void legit_bot::find_target()
{
	...
	for (auto i = 1; i <= m_globals()->m_maxclients; ++i)
	{
		...
		if (!cfg::g_cfg.legitbot.friendly_fire && e->m_iTeamNum() == g_ctx.local()->m_iTeamNum())
			continue;

		Vector engine_angles;
		m_engine()->GetViewAngles(engine_angles);
		targets[i].fov = math::get_fov(engine_angles, math::calculate_angle(g_ctx.globals.eye_pos, e->hitbox_position(CSGOHitboxID::Chest)));
