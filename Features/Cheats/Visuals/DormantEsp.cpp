...
bool c_dormant_esp::adjust_sound(player_t* entity)
{
	...
	if (entity == g_ctx.local())
		type = LOCAL;
	else if (entity->m_iTeamNum() == g_ctx.local()->m_iTeamNum() && !m_cvar()->FindVar("mp_teammates_are_enemies")->GetInt() == 1)
		type = TEAM;

	auto expired = false;

	if (fabs(m_globals()->m_realtime - sound_player.m_iReceiveTime) > cfg::g_cfg.player.type[type].dormant_time)
		expired = true;
