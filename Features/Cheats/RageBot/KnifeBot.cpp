...
void knifebot::run(CUserCmd* cmd)
{
	...
	if (!cfg::g_cfg.ragebot.knife_bot)
		return;
...
static bool compare_targets_distance(const target& first, const target& second)
{


	return first.e->m_vecOrigin().DistTo(g_ctx.local()->m_vecOrigin()) > second.e->m_vecOrigin().DistTo(g_ctx.local()->m_vecOrigin());
}
...
void knifebot::scan_targets()
{
	...
	auto knife_targets = Rbot::get().targets;
	std::sort(knife_targets.begin(), knife_targets.end(), compare_targets_distance);

	for (auto& target : knife_targets)
	{
