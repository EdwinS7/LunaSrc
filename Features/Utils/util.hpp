...
namespace util
{
	...
	static T* FindHudElement(const char* name)
	{
		...
		if (!pThis)
		{
			static auto pThisSignature = util::FindSignature(CLIENT_DLL, crypt_str("B9 ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B 5D 08"));
		...
		static auto find_hud_element = reinterpret_cast<DWORD(__thiscall*)(void*, const char*)>(util::FindSignature(CLIENT_DLL, crypt_str("55 8B EC 53 8B 5D 08 56 57 8B F9 33 F6 39 77 28")));
