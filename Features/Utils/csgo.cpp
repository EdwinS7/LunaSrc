...
IVEngineClient * C_CSGO::m_engine() {
	if (!p_engine)
		p_engine = get_interface<IVEngineClient>(ENGINE_DLL, crypt_str("VEngineClient"));
...
IBaseClientDLL * C_CSGO::m_client() {
	if (!p_client)
		p_client = get_interface<IBaseClientDLL>(CLIENT_DLL, crypt_str("VClient0"));
...
IClientEntityList * C_CSGO::m_entitylist() {
	if (!p_entitylist)
		p_entitylist = get_interface<IClientEntityList>(CLIENT_DLL, crypt_str("VClientEntityList"));
...
IEngineTrace * C_CSGO::m_trace() {
	if (!p_trace)
		p_trace = get_interface<IEngineTrace>(ENGINE_DLL, crypt_str("EngineTraceClient"));
...
IEngineSound* C_CSGO::m_enginesound() {
	if (!p_enginesound)
		p_enginesound = get_interface<IEngineSound>(ENGINE_DLL, crypt_str("IEngineSoundClient"));
...
IVRenderView * C_CSGO::m_renderview() {
	if (!p_renderview)
		p_renderview = get_interface<IVRenderView>(ENGINE_DLL, crypt_str("VEngineRenderView"));
...
IVModelRender * C_CSGO::m_modelrender() {
	if (!p_modelrender)
		p_modelrender = get_interface<IVModelRender>(ENGINE_DLL, crypt_str("VEngineModel"));
...
IVModelInfoClient * C_CSGO::m_modelinfo() {
	if (!p_modelinfo)
		p_modelinfo = get_interface<IVModelInfoClient>(ENGINE_DLL, crypt_str("VModelInfoClient"));
...
IPrediction * C_CSGO::m_prediction() {
	if (!p_prediciton)
		p_prediciton = get_interface<IPrediction>(CLIENT_DLL, crypt_str("VClientPrediction"));
...
IGameMovement * C_CSGO::m_gamemovement() {
	if (!p_gamemovement)
		p_gamemovement = get_interface<IGameMovement>(CLIENT_DLL, crypt_str("GameMovement"));
...
IVDebugOverlay * C_CSGO::m_debugoverlay() {
	if (!p_debugoverlay)
		p_debugoverlay = get_interface<IVDebugOverlay>(ENGINE_DLL, crypt_str("VDebugOverlay"));
...
IGameEventManager2 * C_CSGO::m_eventmanager() {
	if (!p_eventmanager)
		p_eventmanager = get_interface<IGameEventManager2>(ENGINE_DLL, crypt_str("GAMEEVENTSMANAGER002"), true);
...
IViewRenderBeams * C_CSGO::m_viewrenderbeams() {
	if (!p_viewrenderbeams)
		p_viewrenderbeams = *reinterpret_cast<IViewRenderBeams **>(util::FindSignature(CLIENT_DLL, g_ctx.signatures.at(1).c_str()) + 0x1);
...
CGlowObjectManager * C_CSGO::m_glow() {
	if (!p_glow)
		p_glow = *reinterpret_cast<CGlowObjectManager**>(util::FindSignature(CLIENT_DLL, g_ctx.signatures.at(2).c_str()) + 0x3);
...
IMoveHelper * C_CSGO::m_movehelper() {
	if (!p_movehelper)
		p_movehelper = **reinterpret_cast<IMoveHelper***>(util::FindSignature(CLIENT_DLL, g_ctx.signatures.at(3).c_str()) + 0x2);
...
CInput * C_CSGO::m_input() {
	if (!p_input)
		p_input = *reinterpret_cast<CInput**>(util::FindSignature(CLIENT_DLL, g_ctx.signatures.at(4).c_str()) + 0x1);
...
C_CSPlayerResource * C_CSGO::m_playerresource() {
	if (!p_playerresource)
		p_playerresource = **reinterpret_cast<C_CSPlayerResource ***>(util::FindSignature(CLIENT_DLL, g_ctx.signatures.at(5).c_str()) + 0x2);
...
CSGameRulesProxy * C_CSGO::m_gamerules() {
	if (!p_gamerules)
		p_gamerules = **reinterpret_cast<CSGameRulesProxy ***>(util::FindSignature(CLIENT_DLL, g_ctx.signatures.at(6).c_str()) + 0x1);
...
DWORD C_CSGO::m_postprocessing() {
	if (!p_postprocessing)
		p_postprocessing = *(DWORD*)(util::FindSignature(CLIENT_DLL, g_ctx.signatures.at(7).c_str()) + 0x2);
...
DWORD C_CSGO::m_ccsplayerrenderablevftable() {
	if (!p_ccsplayerrenderablevftable)
		p_ccsplayerrenderablevftable = *(DWORD *)(util::FindSignature(CLIENT_DLL, g_ctx.signatures.at(8).c_str()) + 0x4E);
...
NetworkStringTableContainer* C_CSGO::m_networkStringTableContainer()
{
	if (!p_networkStringTableContainer)
		p_networkStringTableContainer = get_interface<NetworkStringTableContainer>(ENGINE_DLL, crypt_str("VEngineClientStringTable"));
...
IWeaponSystem* C_CSGO::m_weaponsys() {
	...
	if (!p_weaponsys)
		p_weaponsys = *reinterpret_cast<IWeaponSystem**>(util::FindSignature(CLIENT_DLL, g_ctx.signatures.at(21).c_str()) + 2);
