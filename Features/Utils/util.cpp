...
#include "util.hpp"
#include "..\cheats\visuals\PlayerEsp.h"
...
namespace util
{
	...
	bool good_backtrack_tick(float m_flSimulationTime) {
		...
		correct += get_interpolation();

		correct = math::clamp(correct, 0.f, 1.f);

		float delta = correct - (m_globals()->m_curtime - m_flSimulationTime);
	...
	void create_state(c_baseplayeranimationstate* state, player_t* e)
	{
		...
		static auto fn = reinterpret_cast <Fn> (util::FindSignature(CLIENT_DLL, crypt_str("55 8B EC 56 8B F1 B9 ? ? ? ? C7 46")));
	...
	void update_state(c_baseplayeranimationstate* state, const Vector& angles)
	{
		...
		static auto fn = reinterpret_cast <Fn> (util::FindSignature(CLIENT_DLL, crypt_str("55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 F3 0F 11 54 24")));
	...
	void reset_state(c_baseplayeranimationstate* state)
	{
		...
		static auto fn = reinterpret_cast <Fn> (util::FindSignature(CLIENT_DLL, crypt_str("56 6A 01 68 ? ? ? ? 8B F1")));
