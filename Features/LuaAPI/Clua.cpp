...
#include <Windows.h>
#include <any>
#include "..\cheats\visuals\OtherEsp.h"
#include "..\utils\csgo.hpp"
#include "..\cheats\misc\logs.h"
#include "..\includes.hpp"
#include "../logging.h"
#include "../Utils/render.h"
#include "../Cheats/Exploits/TickBase.h"
...
namespace ns_client
{
	...
	{
		if (eventname != crypt_str("on_paint") && eventname != crypt_str("on_createmove") && eventname != crypt_str("on_shot") && eventname != crypt_str("pre_resolve"))
		{
...
namespace ns_menu
{
	...
	bool find_config_item(std::string name, std::string type)
	{
		...
		if (config_items.find(name) == config_items.end())
		{
			...
			if (!found)
			{
				eventlogs::get().add(crypt_str("Lua error: cannot find menu variable (find_config_item) \"") + name + '\"', true);
				return false;
			}
		}
	...
	bool get_bool(std::string name)
	{
		...
		if (it.type == NEXT_LINE)
		{
			...
			eventlogs::get().add(crypt_str("Lua error: cannot find menu variable (bool) \"") + name + '\"', true);
			return false;
		}
	...
	int get_int(std::string name)
	{
		...
		if (it.type == NEXT_LINE)
		{
			...
			eventlogs::get().add(crypt_str("Lua error: cannot find menu variable (int) \"") + name + '\"', true);
			return 0;
		}
	...
	float get_float(std::string name)
	{
		...
		if (it.type == NEXT_LINE)
		{
			...
			eventlogs::get().add(crypt_str("Lua error: cannot find menu variable (float) \"") + name + '\"', true);
			return 0.0f;
		}
	...
	Color get_color(std::string name)
	{
		...
		if (it.type == NEXT_LINE)
		{
			...
			eventlogs::get().add(crypt_str("Lua error: cannot find menu variable (clr) \"") + name + '\"', true);
			return Color::White;
		}
...
{
	bool can_doubletap(bool check_charge_status) {
		return exploit::get().CanDoubleTap(check_charge_status);
	}

	int get_shift_ticks() {
		return m_cvar()->FindVar("sv_maxusrcmdprocessticks")->GetInt();
	}
}
...
void c_lua::initialize()
{
	...
	lua[crypt_str("collectgarbage")] = &functions::collectgarbage;
	lua[crypt_str("dofile")] = &functions::dofile;
	lua[crypt_str("load")] = sol::nil;
	lua[crypt_str("loadfile")] = &functions::loadfile;
	lua[crypt_str("pcall")] = &functions::pcall;
	lua[crypt_str("print")] = &ns_client::log;
	lua[crypt_str("xpcall")] = &functions::xpcall;
	lua[crypt_str("getmetatable")] = &functions::getmetatable;
	lua[crypt_str("setmetatable")] = &functions::setmetatable;
	...
	lua.new_usertype<entity_t>(crypt_str("entity"), // new
		...
		(std::string)crypt_str("set_prop_string"), &entity_t::SetPropString
		);
	...
	lua.new_usertype <Vector>(crypt_str("vector3"), sol::constructors <Vector(), Vector(float, float, float)>(),
	...
	lua.new_enum(crypt_str("hitboxes"),
		...
		crypt_str("right_calf"), CSGOHitboxID::RightCalf,
		crypt_str("left_calf"), CSGOHitboxID::LeftCalf,
		...
		crypt_str("right_forearm"), CSGOHitboxID::RightLowerArm,
		crypt_str("left_upper_arm"), CSGOHitboxID::LeftUpperArm,
		crypt_str("left_forearm"), CSGOHitboxID::LeftLowerArm
	);

	lua.new_usertype <player_t>(crypt_str("player"), sol::base_classes, sol::bases<entity_t>(), 
		(std::string)crypt_str("get_index"), &player_t::EntIndex,
		(std::string)crypt_str("is_dormant"), &player_t::IsDormant,
		(std::string)crypt_str("get_team"), &player_t::m_iTeamNum,
		(std::string)crypt_str("is_alive"), &player_t::is_alive,
	...
	lua.new_usertype <weapon_t>(crypt_str("weapon"), sol::base_classes, sol::bases<entity_t>(),
		...
		(std::string)crypt_str("is_non_scope"), &weapon_t::is_non_aim,
	...
	client[crypt_str("register_callback")] = ns_client::add_callback;
	...
	indicators[crypt_str("add")] = ns_indicator::add_indicator;
	indicators[crypt_str("add_position")] = ns_indicator::add_indicator_with_pos;
