...
bool ImGui::ContentTab(const char* label, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (active)
    {
        ...
        ImClamp(it_slider->second, 0.f, 1.f);
    }
    else
    {
        if (it_slider->second > 1.f)
...
bool ImGui::MainTab(const char* label, ImFont* icon_font, const char* icon, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    PushFont(icon_font);
    RenderTextClipped(bb.Min + ImVec2(size_arg.x * 0.05f, -3 ), bb.Max, icon, NULL, &icon_size, ImVec2(0.f, 0.27f), &bb);
    PopFont();
    ...
    PushStyleColor(ImGuiCol_Text, it_text->second);
    RenderTextClipped(bb.Min + ImVec2(size_arg.x * 0.33f, (4 ) - 4), bb.Max, label, NULL, &label_size, ImVec2(0.f, 0.41f), &bb);
    PopStyleColor();
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ...
    it_hover->second = ImClamp(it_hover->second + (3.f * ImGui::GetIO().DeltaTime * ((hovered || clicked) ? 1.f : -1.f)), 0.0f, 1.f);
    ...
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(75 / 255.f, 75 / 255.f, 75 / 255.f, it_hover->second)), 2.f);
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(10, 2), window->DC.CursorPos + size_arg);
    ...
    */
    ...
    const ImVec4 text_act = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f);
    const ImVec4 icon_act = ImVec4(cfg::g_cfg.menu.menu_color_col.r() / 255.f, cfg::g_cfg.menu.menu_color_col.g() / 255.f, cfg::g_cfg.menu.menu_color_col.b() / 255.f, 1.f);
    const ImVec4 dis_clr = ImVec4(173 / 255.f, 42 / 255.f, 46 / 255.f, 1.f);//173, 42, 46


    window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(25, 25, 25, (int)(g.Style.Alpha * 255)), 6);



    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(35 / 255.f, 35 / 255.f, 35 / 255.f, it_hover->second)), 6);

    if (use_icon)
    {
        if (icon_label == "7")
        {
            PushStyleColor(ImGuiCol_TextDisabled, icon_act);
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    //ImGui::SetCursorPos({ 17, 17 });


    const ImRect bb(window->DC.CursorPos + ImVec2(17, 5), window->DC.CursorPos + size + padding * 2);
    ...
    it_hover->second = ImClamp(it_hover->second + (3.0f * ImGui::GetIO().DeltaTime * ((hovered || held) ? 1.f : -1.f)), 0.0f, 1.f);
    ...
    //rgba(52,53,57,255)
    RenderFrame(bb.Min, bb.Max, GetColorU32(ImVec4(52 / 255.f, 53 / 255.f, 57 / 255.f, it_hover->second)), true, ImClamp((float)ImMin(padding.x, padding.y), 8.0f, g.Style.FrameRounding));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(window->DC.CursorPos + ImVec2(6, 4), window->DC.CursorPos + ImVec2(window->Size.x - 56, 14));

    const ImRect frame_bb(window->DC.CursorPos + ImVec2(6, 2), window->DC.CursorPos + ImVec2(window->Size.x - 6, label_size.y + 5));
    ...
    const ImRect text_bb(window->DC.CursorPos + ImVec2(8, 0) + ImVec2(0, style.FramePadding.y), window->DC.CursorPos + ImVec2(8, 0) + ImVec2(0, style.FramePadding.y) + label_size);
    if (label_size.x > 0)
    {
    ...
    ImVec4 text = ImLerp(ImVec4{ 150 / 255.f, 150 / 255.f, 150 / 255.f, g.Style.Alpha }, ImVec4{ 240 / 255.f, 240 / 255.f, 240 / 255.f, g.Style.Alpha }, it_filled->second);


    float active = ImLerp((GetIO().DeltaTime * 6), it_filled->second, 1.f);
    float grabAnim = ImLerp((GetIO().DeltaTime * 6), it_filled->second, 1.f);


    window->DrawList->AddRectFilled({ total_bb.Max.x - 42, total_bb.Min.y }, { total_bb.Max.x - 4, total_bb.Max.y }, ImTricks::Animations::FastColorLerp(ImColor(25, 25, 25), ImColor(15, 15, 15), active), 7);
    window->DrawList->AddCircleFilled({ total_bb.Max.x - 32 + grabAnim * 18, total_bb.Min.y + 8 }, 4.0f, ImTricks::Animations::FastColorLerp(ImColor(cfg::g_cfg.menu.menu_color_col.r(), cfg::g_cfg.menu.menu_color_col.g(), cfg::g_cfg.menu.menu_color_col.b(), 100), ImColor(cfg::g_cfg.menu.menu_color_col.r(), cfg::g_cfg.menu.menu_color_col.g(), cfg::g_cfg.menu.menu_color_col.b()), active));



    if (label_size.x > 0.0f) {
        GetWindowDrawList()->AddText({ total_bb.Min.x + 4, total_bb.Min.y }, ImColor(text), label);
    }
...
bool ImGui::subtab(const char* label, bool selected) {
    ...
    const ImRect bb(pos, { pos.x + 80, pos.y + 25 });
    ItemSize(size, style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;
    ...
    //white
    ImVec4 white = ImVec4{ 158 / 255.f, 157 / 255.f, 161 / 255.f, *lineAlpha };
    //red
    ImVec4 col = ImVec4{ cfg::g_cfg.menu.menu_color_col.r() / 255.f, cfg::g_cfg.menu.menu_color_col.g() / 255.f, cfg::g_cfg.menu.menu_color_col.b() / 255.f, *lineAlpha };

    //line
    //window->DrawList->AddLine({ bb.Min.x - 12, bb.Max.y + 7 }, { bb.Max.x + 12, bb.Max.y + 7 }, ImColor(white), 2);

    //frame
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(35, 35, 35), 4.f, ImDrawCornerFlags_All, 1);
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(col), 4.f, ImDrawCornerFlags_All, 1);

    //text
    window->DrawList->AddText(ImVec2(bb.Min.x + (size.x / 2 - label_size.x / 2), bb.Min.y + (CalcTextSize(label).y / 2) - 3), ImColor(150 / 255.f, 150 / 255.f, 150 / 255.f, *alpha), label);

    return pressed;
}

bool ImGui::tabby(const char* icon, const char* label, bool selected, ImFont* icon_font)
{
    ...
    const ImVec2 label_size = CalcTextSize(icon);
    ...
    const ImRect bb(pos, ImVec2(pos.x + 30, pos.y + 30));
    ItemSize(size, 0);
    if (!ItemAdd(bb, id))
        return false;
    ...
    //ImVec4 text = ImLerp(ImVec4{ 158 / 255.f, 157 / 255.f, 161 / 255.f, GetStyle().Alpha }, ImVec4{ 77 / 255.f, 125 / 255.f, 253 / 255.f, GetStyle().Alpha }, it_filled->second);
    ImColor iconclr = selected ? ImColor(cfg::g_cfg.menu.menu_color_col.r(), cfg::g_cfg.menu.menu_color_col.g(), cfg::g_cfg.menu.menu_color_col.b()) : ImColor(158, 157, 161);
    ImColor box_clr = selected ? ImColor(25, 25, 25) : ImColor(35, 35, 35);
    //ImVec4 text2 = ImLerp(ImVec4{ 158 / 255.f, 157 / 255.f, 161 / 255.f, GetStyle().Alpha }, ImVec4{ 220 / 255.f, 220 / 255.f, 220 / 255.f, GetStyle().Alpha }, it_filled->second);


    auto active = ImColor(46, 46, 46);

    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);


    //bg
    //window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(52, 53, 57, int(255 * it_filled->second)), 11.4f, ImDrawCornerFlags_Right);

    //line 158 / 255.f, 157 / 255.f, 161 / 255.f
    //window->DrawList->AddRectFilled(ImVec2(bb.Min.x + 1, bb.Min.y), ImVec2(bb.Min.x + 1 + it_filled->second * 2.8f, bb.Max.y), ImColor(255, 255, 255, int(225 * it_filled->second)), 9, ImDrawCornerFlags_All);

    //RenderFrame(bb.Min, bb.Max, col, true, 3.f);

    //icon
    window->DrawList->AddRectFilled(bb.Min, bb.Max, box_clr, 8.f, ImDrawCornerFlags_Right | ImDrawCornerFlags_Left);

    ImGui::PushFont(icon_font);
    window->DrawList->AddText(ImVec2(bb.Min.x + 2, bb.Min.y + 2), iconclr, icon);
    ImGui::PopFont();

    //label
    //ImGui::PushFont(CMenu::get().montherrat_12px);//			ImGui::PushFont(montherrat_12px);
    //window->DrawList->AddText(ImVec2(bb.Min.x + 46, bb.Min.y + (CalcTextSize(label).y / 2)), ImColor(text2), label);
    //ImGui::PopFont();
...
bool ImGui::cfgtab(const char* label, bool active, ImVec2 size_arg) {
    //IM_GUI;
    ...
    ImVec2 pos = window->DC.CursorPos + ImVec2(10.0f, -1.f);
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
        pos.y += window->DC.CurrLineTextBaseOffset - style.FramePadding.y;
    ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    if (active) {
        col = GetColorU32(ImVec4(15 / 255.f, 15 / 255.f, 15 / 255.f, *alpha));
        c_col = GetColorU32(ImVec4(240 / 255.f, 240 / 255.f, 240 / 255.f, *alpha));
        t_col = GetColorU32(ImVec4(cfg::g_cfg.menu.menu_color_col.r() / 255.f, cfg::g_cfg.menu.menu_color_col.g() / 255.f, cfg::g_cfg.menu.menu_color_col.b() / 255.f, *alpha));
    }
    else if (!active) {
        col = GetColorU32(ImVec4(35 / 255.f, 35 / 255.f, 35 / 255.f, *alpha));
        c_col = GetColorU32(ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, *alpha));
        t_col = GetColorU32(ImVec4(1.f, 1.f, 1.f, *alpha));
    }

    // GetColorU32(ImVec4(52 / 255.f, 53 / 255.f, 57 / 255.f, it_hover->second))
    ...
    it_hover->second = ImClamp(it_hover->second + (3.0f * ImGui::GetIO().DeltaTime * ((hovered || held || pressed) ? 1.f : -1.f)), 0.0f, 1.f);
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, t_col, 4.f, ImDrawCornerFlags_All, 1);


    //window->DrawList->AddCircleFilled(bb.Min + ImVec2(15, 17.5f), 3.f, ImColor(143, 142, 146, 255), 360);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item) {
    //if (std::string(label).at(0) != '#' && std::string(label).at(1) != '#') {
        //ImGui::SetCursorPosX(9); // ImGui::SetCursorPosY(9);
        //ImGui::Text(label);
    ...
    //const ImRect frame_bb(window->DC.CursorPos + ImVec2(8,  0), window->DC.CursorPos + ImVec2(203, ImMin(26.f, label_size.y + style.FramePadding.y * 2.0f + 5)));


    const ImRect frame_bb(window->DC.CursorPos + ImVec2(6, 2), window->DC.CursorPos + ImVec2(window->Size.x - 6, label_size.y + 25));
    ...
    //RenderFrame(total_bb.Min, total_bb.Max, ImGuiCol_Text, true, 3.f);
    ...
    //label text
    window->DrawList->AddText(frame_bb.Min + ImVec2(4, -2), ImColor(150 / 255.f, 150 / 255.f, 150 / 255.f, g.Style.Alpha), label);
    //box
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        window->DrawList->AddRectFilled(total_bb.Min + ImVec2(4, 15), total_bb.Max + ImVec2(-4, -5), ImColor(25, 25, 25, int(255 * GetStyle().Alpha)), 4, popup_open ? ImDrawCornerFlags_Top : ImDrawCornerFlags_All);
        // window->DrawList->AddRect(total_bb.Min + ImVec2(4, 15), total_bb.Max + ImVec2(-4, -5), ImColor(64, 64, 69, int(255 * GetStyle().Alpha)), 4, popup_open ? ImDrawFlags_RoundCornersTop : ImDrawFlags_RoundCornersAll);
    }

    //Outline
    if (popup_open)
        window->DrawList->AddRect(total_bb.Min + ImVec2(4, 15), total_bb.Max + ImVec2(-4, -5), ImColor(35, 35, 35, int(255 * GetStyle().Alpha)), 4, popup_open ? ImDrawCornerFlags_Top : ImDrawCornerFlags_All);
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            RenderArrow(window->DrawList, total_bb.Max + ImVec2(-23, -20), ImColor(cfg::g_cfg.menu.menu_color_col.r(), cfg::g_cfg.menu.menu_color_col.g(), cfg::g_cfg.menu.menu_color_col.b()), popup_open ? ImGuiDir_Down : ImGuiDir_Up, 0.9f);
    }
    ////inside box text
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        ImVec2 preview_pos = frame_bb.Min + ImVec2(style.FramePadding.x + 7, (CalcTextSize(preview_value).y / 2 + 8));
    ...
    it_open->second = ImClamp(it_open->second + (15.f * ImGui::GetIO().DeltaTime * (popup_open ? 1.f : -1.f)), 0.00f, 1.f);
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(window->Size.x - 20, 0.0f), ImVec2(window->Size.x, it_open->second * CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            SetNextWindowPos(pos + ImVec2(4, -5));
            //SetNextWindowBgAlpha(24);
        }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, bool minimum_damage, float power)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(6, 2), window->DC.CursorPos + ImVec2(window->Size.x - 6, label_size.y + 18));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max);
    ...
    ImRect slider_bb = ImRect(frame_bb.Min + ImVec2(4, label_size.y + 3), frame_bb.Max - ImVec2(7, 8));
    ...
    //bg
     window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(total_bb.Max.x - 4, slider_bb.Max.y + 4), ImColor(35, 35, 35, (int)(g.Style.Alpha * 255)), 6);
    //window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(it_hover->second), g.Style.FrameRounding);
    ...
    //slider
    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(grab_bb.Min.x + 7 + (g.Style.Alpha), slider_bb.Max.y + 4), ImColor(cfg::g_cfg.menu.menu_color_col.r(), cfg::g_cfg.menu.menu_color_col.g(), cfg::g_cfg.menu.menu_color_col.b(), (int)(g.Style.Alpha * 255)), 6);
    //   window->DrawList->AddRectFilled(ImVec2(total_bb.Min.x + (g.Style.Alpha * it_old_value->second) + 3, slider_bb.Min.y - 2), ImVec2(total_bb.Min.x + 8 + (g.Style.Alpha * it_old_value->second), slider_bb.Max.y + 2 + 4), ImColor(231 / 255.f, 231 / 255.f, 231 / 255.f, GetStyle().Alpha), 6);
    //slider dot
    if (hovered)
        window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x + 2, slider_bb.Min.y - 2), ImVec2(grab_bb.Min.x + 7 + (g.Style.Alpha), slider_bb.Max.y + 2 + 4), ImColor(231 / 255.f, 231 / 255.f, 231 / 255.f, GetStyle().Alpha), 6);
    ...
    {
        auto value_damage = "Enemy HP + " + std::to_string(*(int*)p_data - 100);
        value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), "%s", value_damage.c_str());
    }

    //amount text
    window->DrawList->AddText(ImVec2(total_bb.Max.x - 4 - CalcTextSize(value_buf).x, total_bb.Max.y - 32), ImColor(cfg::g_cfg.menu.menu_color_col.r() / 255.f, cfg::g_cfg.menu.menu_color_col.g() / 255.f, cfg::g_cfg.menu.menu_color_col.b() / 255.f, 255 / 255.f), value_buf, value_buf_end);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        RenderNavHighlight(frame_bb, id);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(25, 25, 25, int(255 * GetStyle().Alpha)), 4);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(35, 35, 35, int(255 * GetStyle().Alpha)), 4);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = GetColorU32(ImVec4(35 / 255.f, 35 / 255.f, 35 / 255.f, 1.f)); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), GetColorU32(ImVec4(150 / 255.f, 150 / 255.f, 150 / 255.f, 255 / 255.f)));
...
bool ImGui::ColorEdit(const char* label, Color* newcolor, bool keybind, ImGuiColorEditFlags flags, bool scripts)
{
    ImGui::SameLine();
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        //window->DC.CursorPos = ImVec2(total_bb.Max.x - 42 + button_offset_x, total_bb.Max.y);
        ...
        //const ImRect total_bb(frame_bb.Min, frame_bb.Max);
        //
        //window->DrawList->AddRectFilled({ total_bb.Max.x - 42, total_bb.Min.y }, { total_bb.Max.x - 4, total_bb.Max.y }, ImTricks::Animations::FastColorLerp(ImColor(55, 55, 57), ImColor(78, 78, 80), active), 7);

        //ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 26);

        if (ColorButton("ColorButtons", col_v4, keybind, flags, ImVec2(27, 16)))
        {
        ...
        ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_ColorPicker | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;
        ...
        if (BeginPopup("picker", window_flags))
        {
            ...
            //PushStyleColor(ImGuiCol_FrameBg, ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 255.f));
    ...
    if (!keybind && label_size.x > 0.0f) {
        GetWindowDrawList()->AddText({ total_bb.Min.x + 4, total_bb.Min.y }, ImColor(150, 150, 150), label);
    }
    ...
    if (cfg::g_cfg.scripts.developer_mode && ImGui::IsItemHovered())
    {
        for (auto& item : cfg_manager->items)
        {
            if (newcolor == item->pointer)
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, bool keybind, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if (keybind)
        ImGui::SetCursorPosX(window->Size.x - 83);
    else
        ImGui::SetCursorPosX(window->Size.x - 37);
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(6, 2), window->DC.CursorPos + size);
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col.w < 1.0f)
    {
        ...
        //RenderFrame(bb_inner.Min, bb_inner.Max, ImGuiCol_Text, true, 26);
        //window->DrawList->AddRect(bb_inner.Min, bb_inner.Max, ImColor(75, 75, 75, (int)(g.Style.Alpha * 255)), 26);
    }
    else
    {
        RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_rgb), grid_step, ImVec2(off, off), 26);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImColor t_col = selected ? ImColor(cfg::g_cfg.menu.menu_color_col.r(), cfg::g_cfg.menu.menu_color_col.g(), cfg::g_cfg.menu.menu_color_col.b(), int(255 * g.Style.Alpha)) : ImColor(150, 150, 150, int(255 * g.Style.Alpha));
...
bool BeginChildSkeet(const char* str_id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    return ImGui::BeginChildEx(str_id, window->GetID(str_id), size_arg, border, extra_flags);
}
...
bool BeginChildFrameSkeet(const char* label, const ImVec2& size, ImGuiWindowFlags extra_flags)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    bool ret = BeginChildSkeet(label, size, true, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysUseWindowPadding | extra_flags);
    return ret;
}
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    // Size default to hold ~7 items. Fractional number of items helps seeing that we can scroll down/up without looking at scrollbar.
    ImVec2 size = CalcItemSize(size_arg, window->Size.x - 5, GetTextLineHeightWithSpacing() * 7.4f + style.ItemSpacing.y);
    ImVec2 frame_size = ImVec2(size.x, ImMax(size.y, label_size.y));
    ...
    BeginChildFrameSkeet(label, frame_bb.GetSize(), ImGuiWindowFlags_SkinList);
    return true;
}
