...
ImGuiStyle::ImGuiStyle()
{
    ...
    WindowRounding = 0.0f;             // main bg rounding
    WindowBorderSize = 1.0f;             // Thickness of border around windows. Generally set to 0.0f or 1.0f. Other values not well tested.
    WindowMinSize = ImVec2(15, 15);    // Minimum window size
    ...
    ItemSpacing = ImVec2(8, 10);        // Horizontal and vertical spacing between widgets/lines
...
void ImGui::UpdateMouseWheel()
{
    ...
    if (wheel_y != 0.0f && !g.IO.KeyCtrl)
    {
        ...
        if (!(window->Flags & ImGuiWindowFlags_NoScrollWithMouse) && !(window->Flags & ImGuiWindowFlags_NoMouseInputs))
        {
            ...
            float scroll_step = ImFloor(ImMin(window->CalcFontSize(), max_step));
...
bool ImGui::BeginMainChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
...
bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    child_window->DC.CursorPos += ImVec2(10, 0);
    // Process navigation-in immediately so NavInit can run on first frame
    if (g.NavActivateId == id && !(flags & ImGuiWindowFlags_NavFlattened) && (child_window->DC.NavLayerActiveMask != 0 || child_window->DC.NavHasScroll))
    {
        FocusWindow(child_window);
    ...
    static ImColor normal = ImColor(27, 27, 27);
    static ImColor normal2 = ImColor(35, 35, 35, 255);
    static ImColor normal3 = ImColor(35, 35, 35, 0);
    static ImColor normal4 = ImColor(0, 0, 0, 100);
    static ImColor normal5 = ImColor(0, 0, 0, 0);
    ImVec4 text2 = ImVec4{ 150 / 255.f, 150 / 255.f, 150 / 255.f, GetStyle().Alpha };

    //bg
    parent_window->DrawList->AddRectFilled(pos - ImVec2(0, 30), pos + size_arg, normal, 4);

    //frame
    parent_window->DrawList->AddRect(pos - ImVec2(0, 30), pos + size_arg, ImColor(35, 35, 35, 255), 4);

    //gradient
    parent_window->DrawList->AddRectFilledMultiColor(pos - ImVec2(0, 5), pos + ImVec2(size_arg.x / 2, -4), normal2, normal3, normal3, normal2);
    parent_window->DrawList->AddRectFilledMultiColor(pos + ImVec2(size_arg.x, -5), pos + ImVec2(size_arg.x / 2, -4), normal2, normal3, normal3, normal2);
    parent_window->DrawList->AddRectFilledMultiColor(pos + ImVec2(0, -4), pos + ImVec2(size_arg.x, 6), normal4, normal4, normal5, normal5);

    //header
    PushFont(CMenu::get().HEADER);
    parent_window->DrawList->AddText({ ImGui::GetWindowPos().x + 10, ImGui::GetWindowPos().y - 27 }, ImColor(text2), name);
    PopFont();

    Spacing(); return ret;
}

bool ImGui::BeginMainChild(const char* str_id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    return BeginMainChildEx(str_id, window->GetID(str_id), size_arg, border, extra_flags);
}
...
void ImGui::RenderMainWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            //ImGui::GetIO().menubg[4] = menu_bg_clr[4];


            //bg
            window->DrawList->AddRectFilled(ImVec2(window->Pos), ImVec2(window->Pos.x + window->Size.x, window->Pos.y + window->Size.y), ImColor(15, 15, 15), 14.0f);

            //child background
            window->DrawList->AddRectFilled({ window->Pos.x + 55, window->Pos.y }, { window->Pos.x + ImGui::GetCurrentWindow()->Size.x, window->Pos.y + ImGui::GetCurrentWindow()->Size.y - 20 }, ImColor(20, 20, 20), 6, 6);

            //inner frame
            window->DrawList->AddRect({ window->Pos.x + 55, window->Pos.y }, { window->Pos.x + ImGui::GetCurrentWindow()->Size.x, window->Pos.y + ImGui::GetCurrentWindow()->Size.y - 20 }, ImColor(35, 35, 35), 6, 6);
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        static auto main_bg_clr = ImColor(255, 255, 255);

        if (orig == true)
            main_bg_clr = ImColor(40, 41, 45);
        else
            main_bg_clr = ImColor(52, 53, 57);

        if (flags & ImGuiWindowFlags_Watermark)
        {
            //If you know what cheat this is you know, Luna is pasted off of it.
            //Hint, it starts with an S. -edwin777/PainFull
            window->DrawList->AddRectFilled(ImVec2(window->Pos), ImVec2(window->Pos.x + window->Size.x, window->Pos.y + window->Size.y), ImColor(40, 41, 45, (int)(GetStyle().Alpha * 174)), 6, ImDrawCornerFlags_All);
            ...
            {
                //bg
                window->DrawList->AddRectFilled(ImVec2(window->Pos), ImVec2(window->Pos.x + 92, window->Pos.y + 25), ImColor(40, 41, 45), 6);
                //logo text
                PushFont(CMenu::get().mainfont_bigger);
                window->DrawList->AddText(ImVec2(window->Pos.x + 15, window->Pos.y + 4), ImColor(233, 233, 233, (int)(255 * GetStyle().Alpha)), "LUNA");
                PopFont();
        ...
        {
            //expandable bg
            window->DrawList->AddRectFilled(ImVec2(window->Pos.x, window->Pos.y + 25),
                ImVec2(window->Pos.x + window->Size.x, window->Pos.y + window->Size.y), ImColor(33, 33, 33, (int)(GetStyle().Alpha * 125.f)), 6, ImDrawCornerFlags_Bot);

            //Background bar
            window->DrawList->AddRectFilled(ImVec2(window->Pos), ImVec2(window->Pos.x + window->Size.x, window->Pos.y + 25), main_bg_clr, 6, ImDrawCornerFlags_Top);

            //line
            //window->DrawList->AddRect(ImVec2(window->Pos), ImVec2(window->Pos.x + window->Size.x, window->Pos.y + 25), ImColor(125, 125, 125, 255), 6, ImDrawCornerFlags_Top);
            ...
            {
                PushFont(CMenu::get().ugen);
                window->DrawList->AddText(ImVec2(window->Pos.x + 11, window->Pos.y + 25 / 2 - CalcTextSize("E").y / 2), ImColor(233, 233, 233, (int)(255 * GetStyle().Alpha)), "E");
                PopFont();
                ...
                PushFont(CMenu::get().mainfont_bigger);
                window->DrawList->AddText(ImVec2(window->Pos.x + 39 + text_x, window->Pos.y + 25 / 2 - CalcTextSize("keybinds").y / 2), ImColor(233, 233, 233, (int)(255 * GetStyle().Alpha)), "keybinds");
                PopFont();
            }
            else
            {
                PushFont(CMenu::get().ugen);
                window->DrawList->AddText(ImVec2(window->Pos.x + 13, window->Pos.y + 25 / 2 - CalcTextSize("F").y / 2), ImColor(233, 233, 233, (int)(255 * GetStyle().Alpha)), "F");
                PopFont();
                ...
                PushFont(CMenu::get().mainfont_bigger);
                window->DrawList->AddText(ImVec2(window->Pos.x + 38 + text_x, window->Pos.y + 25 / 2 - CalcTextSize("spectators").y / 2), ImColor(233, 233, 233, (int)(255 * GetStyle().Alpha)), "spectators");
                PopFont();
            }
        ...
        */
        ...
        {
            window->DrawList->AddRectFilled(window->Pos + ImVec2(0, 0), window->Pos + window->Size, ImColor(25, 25, 25), 4.f, ImDrawCornerFlags_Bot /*| ImDrawCornerFlags_Top*/);
            window->DrawList->AddRect(window->Pos + ImVec2(0, 0), window->Pos + window->Size, ImColor(35, 35, 35), 4.f, ImDrawCornerFlags_Bot /*| ImDrawCornerFlags_Top*/);
            return;
        }
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground))
        {

            {
                ...
                if (override_alpha)
                    bg_col = (bg_col & ~IM_COL32_A_MASK) | (IM_F32_TO_INT8_SAT(alpha) << IM_COL32_A_SHIFT);
                window->DrawList->AddRectFilled(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size, bg_col, window_rounding, (flags & ImGuiWindowFlags_NoTitleBar) ? 0 : ImDrawCornerFlags_Bot | ImDrawCornerFlags_Top);
            }
        }
